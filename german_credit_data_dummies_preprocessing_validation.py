...
cols_drop_dummy_base = ["Dummy_status_checking_account_2", "Dummy_credit_history_0", \
    "Dummy_savings_account/bonds_1", "Dummy_present_employment_2"]
...
cols_baseline_dummies = list(cols_dummies.keys()) + list(cols_numerical.keys())

#with open("german_credit_data/models/cols_baseline_dummies.pickle", "wb") as file:
#    pickle.dump(cols_baseline_dummies, file)

# Run a logistic regression model
y, X = data_train[list(cols_target.keys())[0]], sm.add_constant(data_train[cols_baseline_dummies])
...
#smpickle.save_pickle(res_reg_logit, "german_credit_data/models/baseline_dummies_reg_logit.pickle")

print("The accuracy score for logistic regression on train dataset \n", \
    accuracy_score(y, pred_y_reg_logit))

# Perform model validation
y_val, X_val = data_val[list(cols_target.keys())[0]], sm.add_constant(data_val[cols_baseline_dummies])
